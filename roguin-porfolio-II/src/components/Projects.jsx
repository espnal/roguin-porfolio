...
export default function Projects() {
  const projects = [
    ...
    },
    {
      ...
      img: quiz,
      technologies: ["TypeScript", "JavaScript", "CSS", "React"],
    },
    {
      name: "Recipes App",
      ...
      <ul>
        {projects.map((p, i) => (
          <li
            ...
            {/* Thumbnail - Now takes more space and is centered */}
            <div className="sm:col-span-4 sm:order-1">
              <img
                src={p.img}
                alt={p.name}
                className="w-full h-64 object-cover rounded-lg border-2 border-slate-200/10 
             transition group-hover:border-slate-200/30"
              />
            </div>

            {/* Content - Now takes remaining space */}
            <div className="z-10 order-2 sm:col-span-4">
              <h3>
                <a
                  href={p.link}
                  target="_blank"
